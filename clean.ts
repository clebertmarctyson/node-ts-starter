...
async function cleanProject(): Promise<void> {
  ...
  console.log('');
  const removeTests = keepTests.toLowerCase().trim() !== 'y' && keepTests.toLowerCase().trim() !== 'yes';
  ...
  console.log('');
  if (installPackages.toLowerCase().trim() === 'y' || installPackages.toLowerCase().trim() === 'yes') {
    console.log('\nInstalling packages...\n');
